...
| Files panel | none built in; the owner used the third-party `herdr-sidebar` plugin | `tend files`, docked on the left by prefix+f or the pane menu, in herdr-sidebar's three views (1 files, 2 search, 3 changes): tree with git letters; text search through `git grep` (tracked and new files, not ignored ones; case, whole word, regex, include/exclude globs; live, a moment after typing stops, off the draw loop), opening the editor on the line; changes with diff, stage, commit, discard (twice); find a file by name (`/`, ctrl+p); open in `$EDITOR` in a tab | tend's own, in the one binary, running where the files are |
