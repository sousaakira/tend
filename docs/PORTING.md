...
| Errors (GlitchTip) | none | prefix+E or the toolbar's Errors: the errors a GlitchTip server collected (`internal/glitchtip`, Sentry's web API: organizations, projects, `organizations/{org}/issues/?query=is:<status>`, an issue's `events/latest/`, `PUT issues/{id}/` for its status). The client talks to it itself — a web service, not the projects' machine — with the server and an API token given in the panel's connect box, tested before they are kept, and written to the settings file (0600; `TEND_GLITCHTIP_TOKEN` also serves). Several servers are kept, a table each (`[errors.sources.<host>]`, beside the first one's `[errors] url/token`), so adding or removing one edits only its own lines: the gear or ctrl+k opens the servers box (use, add, remove — the table goes with its token, after enter confirms), a line of server chips and ctrl+g move between them, and `[errors] source` remembers the one shown. A project folder is tied to a server and a GlitchTip project (`projects = { "<folder>" = "org/slug" }` in that server's table, ctrl+l or the title's "link … here"): the panel opened from a pane working under the folder — its program's directory, from pane.get, and the repository it is in, a worktree counting as the repository it came from — shows that server and project whatever was remembered, the deepest tied folder winning. A line of project chips and the unresolved/resolved/ignored presets over a search; enter opens an error with its exception chain, the application's own frames marked with their code line, the request, tags and breadcrumbs. f types it into the pane the panel was opened from — its in-app frames last call first, the request, the environment, and that the paths are the server's — through the context, inert and unsubmitted; w starts the agent in a worktree `fix-<short id>` of the space's project; c puts it in the context; r, i, u resolve, ignore, reopen it — and from the list, without opening it, ctrl+x or the Resolve and Ignore buttons (Reopen, listing the resolved or ignored). An issue is closed from its list the same way, ctrl+x or Close issue, asking completed or not planned. Checked reading the owner's server (53 unresolved in 3 organizations); resolving was checked against a stand-in only | the owner's: the errors of the systems running, found and fixed from where the agents are |
