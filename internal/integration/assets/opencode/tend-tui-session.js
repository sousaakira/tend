...
// TEND_INTEGRATION_ID=opencode-tui
// TEND_INTEGRATION_VERSION=12

import net from "node:net";

const SOURCE = "tend:opencode";
...
export default {
  id: "tend.opencode.session-selection",
  ...
  tui: async (api) => {
    if (
      process.env.TEND_ENV !== "1" ||
      !process.env.TEND_SOCKET_PATH ||
      !process.env.TEND_PANE_ID
    ) {
      return;
    }
...
function setup(api) {
  if (process.env.TEND_ENV !== "1" || !process.env.TEND_SOCKET_PATH || !process.env.TEND_PANE_ID) return;
