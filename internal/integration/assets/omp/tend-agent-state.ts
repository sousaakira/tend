...
const TEND_ENV = process.env.TEND_ENV;
const socketPath = process.env.TEND_SOCKET_PATH;
const socketEndpoint =
  process.platform === "win32" && socketPath ? `\\\\.\\pipe\\${socketPath}` : socketPath;
const paneId = process.env.TEND_PANE_ID;
const source = "tend:omp";
...
function enabled() {
  return TEND_ENV === "1" && !!socketPath && !!paneId && !nestedOmpSession;
}
...
const idleDebounceMs = parseDurationEnv("TEND_OMP_IDLE_DEBOUNCE_MS", 250);
const retryGraceMs = parseDurationEnv("TEND_OMP_RETRY_GRACE_MS", 2500);
...
export default function (pi) {
  ...
  pi.events.on("tend:blocked", (data) => {
    if (!rootSession) {
      return;
    }
    if (!data?.active) {
